...
6. `Desofuscar` vuelve a validar el archivo, pide una ruta de salida y ejecuta oculto el detector BDVE automatico incluido. La salida se remultiplexa sin recodificar y se decodifica completa antes de entregarse.
...
- El preload expone solo `scan`, `reveal`, `copyPath`, `deobfuscate` y `emptyFolder`.
...
- La desofuscacion usa el `ffprobe` empaquetado y el `ffmpeg`/`libvecrptor.dll` de la instalacion local de CapCut. Escribe primero archivos temporales junto a la salida y solo reemplaza el destino despues de validar toda la decodificacion.
...
`test:coverage` exige 100% de lineas, ramas, funciones y sentencias sobre el nucleo de deteccion, analisis, desofuscacion, vaciado y formateo. La suite de componentes cubre los estados visibles y el smoke E2E recorre la aplicacion Electron real con un arbol CapCut sintetico, incluidas las cancelaciones seguras.
