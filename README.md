...
6. `Desofuscar` vuelve a validar el archivo y ejecuta oculto el detector BDVE automatico incluido. El destino inicial es `%USERPROFILE%\Videos\Cortos`, pero puede cambiarse con el selector nativo y queda guardado para las proximas aperturas. El nombre es opcional, completa `.mp4` y agrega `(2)`, `(3)`, etc. si ya existe otro archivo.
